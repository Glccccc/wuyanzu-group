...
<!-- by 管立超 -->
1. **编写 orth_triangle 函数**   

    *功能：* 从所有电阻系列中选择与给定电阻值最接近的标称值。
    
    *参数：* 一个整数，表示电阻值，在 100 到 999 之间（包含 100 和 999）。
    
    *返回值：* 字典或 None。字典的键为 'E6'、'E12'、'E24'、'E48'、'E96'，值为包含一个或多个整数的元组，表示选定的电阻标称值。
    ...
    *测试结果：*  
    ![alt text](asset/2023.05.21/image-1.png)
...
2. **编写 deck 生成器函数**
    ...
    *返回值：* 生成器对象。每次迭代返回一个元组，包含一个整数（表示牌的点数：2 到 14，其中 11 代表 J，12 代表 Q，13 代表 K，14 代表 A）和一个字符串（表示牌的花色：' черви '、' бубны '、' пики '、' трефы '）。
    ...
    *测试结果：*  
    ![alt text](asset/2023.05.21/image-2.png)
3. **编写 math_function_resolver 高阶函数**
   
    *功能：* 计算各种数学函数的四舍五入值。
    
    *参数：*   
    - 一个数学函数，作为位置参数传递，该函数应接受一个位置参数 x。
    - 任意数量的位置参数，表示传递给数学函数的 x 值。
    - 一个布尔类型的关键词参数，用于切换返回值类型，默认为 False（返回浮点数），若为 True 则返回字符串。

    *返回值：* 列表，包含根据 x 值计算出的数学函数值，值的类型取决于关键词参数的设置。
    ...
    *测试结果：* 
    ![alt text](asset/2023.05.21/image-3.png)
4. **编写 repeat 装饰器**
    ...
    *返回值：* 装饰后的函数对象，执行一次该对象将导致被装饰函数执行十次。
    ...
    *测试结果：* 
    ![alt text](asset/2023.05.21/image-4.png)
5. **编写 logger 装饰器**

    *功能：* 记录被装饰函数的调用情况，包括函数名、传递的参数及默认值，若发生异常则记录异常信息。
    
    *参数：*   被装饰函数。

    *返回值：* 装饰后的函数对象，在执行时会在标准输出流中记录相关调用信息和异常信息（若发生）。
    <!-- by 管立超 -->
...
<!-- 结束2023.05.28 readme 文档（郑锐滨） -->

<!-- 2023.06.04 readme 文档（郑锐滨） -->
